"""
DeepCubeA-style value network + beam search for 4x4 Rubik's cube.
...
Usage:
    python value_search_4x4.py [--hours 12] [--device cuda]

Adapted from value_search_3x3.py. Key differences:
- 96 stickers (6 faces x 4x4) instead of 54
- 36 moves (regular + wide) instead of 18
- Deeper curriculum (3->50) and longer training
- 8 mixer layers instead of 6
"""
...
def encode_cube_batch(cubes: list[Cube]) -> torch.LongTensor:
    """Encode a batch of cubes. Returns (B, 96) tensor."""
    batch = []
    for cube in cubes:
        indices = []
        for face in FACE_ORDER:
...
class DistanceNet4x4(nn.Module):
    ...
    def __init__(
        ...
        # Precompute face indices and position features for 96 stickers
        face_indices = []
        pos_features = []
        for face_idx, face in enumerate(FACE_ORDER):
            for r in range(4):
                for c in range(4):
                    ...
                    # Normalize row, col to [-1, 1] for 4x4 grid
    ...
    def forward(self, stickers: torch.Tensor) -> torch.Tensor:
        ...
        # Color embedding: (B, 96, color_embed_dim)
        color_emb = self.color_embed(stickers)

        # Face embedding: (B, 96, face_embed_dim) - same for all batch items
        face_emb = self.face_embed(self.face_indices).unsqueeze(0).expand(B, -1, -1)

        # Position features: (B, 96, pos_dim)
        pos_emb = self.pos_proj(self.pos_features).unsqueeze(0).expand(B, -1, -1)

        # Concatenate and project
        x = torch.cat([color_emb, face_emb, pos_emb], dim=-1)  # (B, 96, input_dim)
...
def sample_walk_endpoints_4x4(
    ...
    Returns: (states, distances) tensors of shape (batch_size, 96) and (batch_size,).
    """
    all_indices = []
    all_dists = []
...
def generate_dcba_targets(
    ...
    2. For each state, expand all 36 children
    3. Target = 1 + min(model(children))  (cost-to-go bootstrap)
    4. Solved states get target = 0

    Returns: (states, targets) where states is (B, 96) and targets is (B,).
    """
    model.eval()
    ...
    all_states = encode_cube_batch(cubes).to(device)  # (B', 96)
    B = len(cubes)
    ...
    child_states = encode_cube_batch(child_cubes).to(device)  # (B'*36, 96)

    # Process in chunks to avoid OOM (36 children per state is more than 18)
    chunk_size = 4096
    all_child_values = []
    ...
    child_values = torch.cat(all_child_values, dim=0)  # (B'*36,)
    child_values = child_values.view(B, N_MOVES_4x4)  # (B, 36)
...
def evaluate_solver(
    ...
    for i in range(num_cubes):
        # 4x4 scrambles use both regular and wide moves (max_depth=1, max_width=2)
        scramble = random_scramble(4, scramble_length, rng, max_depth=1, max_width=2)
        cube = Cube(4)
        cube.apply_moves(scramble)

        solved, history, nodes = beam_search_4x4(
...
def train(args):
    ...
    if use_wandb:
        wandb.init(
            project="rubiks-value-search-4x4",
            config=vars(args),
            name=f"dcba-4x4-{args.d_model}d-{args.n_layers}L",
        )
    ...
    while True:
        ...
        if use_bootstrap:
            ...
            states, targets = sample_walk_endpoints_4x4(
...
def parse_args():
    parser = argparse.ArgumentParser(description="DeepCubeA-style value+search for 4x4 Rubik's cube")
    ...
    parser.add_argument("--n_layers", type=int, default=8, help="Number of mixer layers")
    ...
    parser.add_argument("--curriculum_end", type=int, default=50, help="Final max walk depth")
    ...
    parser.add_argument("--eval_scramble_length", type=int, default=40, help="Scramble length for eval")
    parser.add_argument("--eval_max_steps", type=int, default=500, help="Max beam search steps")
    parser.add_argument("--eval_beam_widths", type=int, nargs="+", default=[1, 8, 32, 128],
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints/value_4x4",
