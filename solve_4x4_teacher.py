#!/usr/bin/env python3
"""
Teacher-based 4x4 center solver.

Uses the dwalton 4x4 solver as a teacher to generate training data,
then trains a small MLP policy+value network to solve 4x4 centers.

Usage:
  python solve_4x4_teacher.py --generate --num-cubes 500
  python solve_4x4_teacher.py --train --hours 2
  python solve_4x4_teacher.py --eval
"""
...
NUM_COLORS = 6
NUM_CENTER_STICKERS = 24  # 4 per face x 6 faces
...
def extract_center_stickers(cube: Cube) -> list[int]:
    """Extract 24 center sticker color indices from a 4x4 cube.
    For each face, the 4 center stickers are grid[1][1], grid[1][2], grid[2][1], grid[2][2].
    """
    indices = []
    for face in FACE_ORDER:
        grid = cube.face_grid(face)
        indices.append(COLOR_TO_IDX[grid[1][1]])
        indices.append(COLOR_TO_IDX[grid[1][2]])
        indices.append(COLOR_TO_IDX[grid[2][1]])
        indices.append(COLOR_TO_IDX[grid[2][2]])
    return indices
...
def generate_data(num_cubes: int, seed: int = 42) -> dict:
    ...
    all_states = []      # list of 24-int lists
...
class CenterPolicyValueNet(nn.Module):
    """Small MLP for 4x4 center solving.

    Input: 24 center sticker color indices -> embed -> flatten -> FC layers
    Output: policy (36-way) + value (scalar distance estimate)
    """

    def __init__(self, embed_dim: int = 32, hidden_dim: int = 256, n_layers: int = 4,
                 dropout: float = 0.1):
        super().__init__()
        self.embed = nn.Embedding(NUM_COLORS, embed_dim)
        input_dim = NUM_CENTER_STICKERS * embed_dim  # 24 * 32 = 768
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Args:
            x: (B, 24) LongTensor of color indices
        Returns:
            policy_logits: (B, 36)
            value: (B,) predicted distance to centers-done
        """
        emb = self.embed(x)           # (B, 24, embed_dim)
        emb = emb.view(emb.size(0), -1)  # (B, 24*embed_dim)
...
def solve_greedy(cube: Cube, model: CenterPolicyValueNet, device: torch.device,
                 max_steps: int = 50) -> tuple[bool, list[Move], int]:
    """Solve centers by greedily following the policy's top prediction."""
    current = cube.copy()
    moves = []
    for step in range(max_steps):
        if centers_done_444(current):
        ...
        moves.append(move)
    return centers_done_444(current), moves, max_steps


def solve_beam(cube: Cube, model: CenterPolicyValueNet, device: torch.device,
               beam_width: int = 32, max_steps: int = 50, top_k: int = 5
...
def main():
    parser = argparse.ArgumentParser(description="4x4 center solver via teacher distillation")
